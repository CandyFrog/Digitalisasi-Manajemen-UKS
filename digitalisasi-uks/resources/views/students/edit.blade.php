<x-app-layout>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden max-w-2xl">
        <div class="p-6 border-b border-gray-100">
            ...
            <p class="text-sm text-gray-500 mt-1">Silakan ubah data siswa di bawah ini.</p>
        </div>
        ...
        <form action="{{ route('students.update', $student->id) }}" method="POST" class="p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
                <div>
                    ...
                    <input type="text" name="nis" id="nis" value="{{ old('nis', $student->nis) }}" class="w-full rounded-lg border-gray-300 focus:border-emerald-500 focus:ring focus:ring-emerald-200 focus:ring-opacity-50 transition duration-150" required>
                    @error('nis')
                ...
                <div>
                    ...
                    <input type="text" name="nama" id="nama" value="{{ old('nama', $student->nama) }}" class="w-full rounded-lg border-gray-300 focus:border-emerald-500 focus:ring focus:ring-emerald-200 focus:ring-opacity-50 transition duration-150" required>
                    @error('nama')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
